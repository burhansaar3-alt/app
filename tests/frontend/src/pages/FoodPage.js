        ...
        {filteredProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredProducts.map((product) => (
              <div
                key={product.id}
                className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-2xl hover:ring-2 hover:ring-orange-500 hover:-translate-y-1 transition-all duration-300 cursor-pointer group"
              ...
              >
                <div className="aspect-square bg-gray-100 overflow-hidden relative">
                  ...
                  {user && (
                    ...
                    >
                      <Heart className={`w-5 h-5 ${wishlistIds.includes(product.id) ? 'fill-orange-500 text-orange-500' : 'text-gray-400'}`} />
                    </button>
                  )}
                </div>
                <div className="p-4">
                  <h3 className="text-sm font-semibold text-gray-900 mb-3 line-clamp-2 group-hover:text-orange-600 transition-colors">{product.name}</h3>
                  <div className="bg-gradient-to-l from-orange-500 via-orange-600 to-gray-800 rounded-xl p-3 flex items-center justify-between group-hover:shadow-lg transition-all">
                    <div className="bg-orange-400 p-2 rounded-lg">
                      <ShoppingCart className="w-5 h-5 text-white" />
                    </div>
                    <div className="text-right">
                      <span className="text-[10px] text-orange-200 block">السعر</span>
                      <span className="text-white text-lg font-bold">{product.price?.toLocaleString()}</span>
                      <span className="text-orange-200 text-xs mr-1">ل.س</span>
                    </div>
                  </div>
                </div>
              </div>
